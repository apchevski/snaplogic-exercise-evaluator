...
description: /grade resolves every exercise to PASS / FAIL / MISSING and assigns 0-10 points (or null for MISSING). FAIL splits into procedural (0 pts, no AI) and output-mismatch (AI judges for partial credit). Point deductions are written into the rule files (general_evaluation_rules.md + per-exercise notes.md). The AI judge never invents a deduction value.
scope: skill:grade
---
...
| **MISSING** | No matching pipeline, OR no output uploaded to SLDB (`csv_output_present` 404), OR no Triggered Task with the convention name (`triggered_task_exists`) | `null` (not graded; excluded from totals) |
...
In all three, the submission can't be graded at all, so it's excluded
from totals rather than scored as 0/10. This keeps the average
meaningful for the exercises a student actually attempted.
...
total as `points_earned / points_possible` where `points_possible` only
counts PASS+FAIL exercises (MISSING is excluded from the denominator).
